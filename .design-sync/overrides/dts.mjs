// forked from design-sync lib/dts.mjs — propsBodyFor's call-signature fallback
// only looked in the barrel entry, which this repo (an app, not a library) has
// no equivalent of, so every shadcn/ui primitive shipped an empty contract.
//
...
export function propsBodyFor(name, ctx) {
  ...
  if (!decl) {
    let decls = project.getSourceFile(entry)?.getExportedDeclarations().get(name) ?? [];
    // FORK: `entry` is <pkgDir>/index.d.ts, which only exists for packages that
    // ship a barrel. Without one this lookup is ALWAYS empty, so every component
    // lacking an explicit <Name>Props interface returned null — here that was all
    // 63 shadcn/ui primitives (they declare props inline as
    // `forwardRef<ElementRef<typeof X>, ComponentPropsWithoutRef<typeof X>>`).
    // Scan the package's own .d.ts files for the export instead. Same file
    // filters as the <Name>Props search above, so a stale copy under
    // deprecated/legacy/experimental can't shadow the live declaration.
    if (!decls.length) {
      for (const sf of project.getSourceFiles()) {
        const fp = sf.getFilePath();
        if (!fp.startsWith(pkgDir)) continue;
        if (/\/(deprecated|legacy|experimental)\//i.test(fp)) continue;
        const found = sf.getExportedDeclarations().get(name);
        if (found?.length) { decls = found; break; }
      }
    }
