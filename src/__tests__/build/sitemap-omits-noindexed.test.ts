 ...
 * built HTML.
 *
 * `describeDist` like its neighbours: this runs only against dist/, which only ci.yml
 * produces before the tests. A dist-less local run skips it, and that skip is not a pass;
 * under `EXPECT_DIST` (set only by ci.yml) it fails instead (GH#159).
 */
...
describeDist('the sitemap does not advertise a noindexed route', built, () => {
